...
public class Renderer {
	...
	private ColorRGB illuminate(Scene scene, SceneObject object, Vector3 P, Vector3 N, Vector3 O) {
		...
		for (int i = 0; i < pointLights.size(); i++) {
			...
			boolean intersect_with_object = false;
			RaycastHit hit = scene.findClosestIntersection(shadowRay);
			if(hit.getDistance()<distanceToLight){
				// there is an object between light and object
				intersect_with_object = true;
			}

			if (!intersect_with_object){
				// TODO: Calculate ColorRGB diffuse and ColorRGB specular terms
				double max_d = Math.max(0,N.normalised().dot(L));
				ColorRGB diff = C_diff.scale(k_d).scale(I).scale(max_d);
				double max_s = Math.max(0,R.dot(V));
				ColorRGB spec = C_spec.scale(k_s).scale(I).scale(Math.pow(max_s, alpha));
				// TODO: Add these terms to colourToReturn
				colourToReturn = colourToReturn.add(spec).add(diff);
			}
