...
public class Tick1Tests {
	...
	private void runTestsPart1() {
		...
		start("illumination: cos(theta)<0");
		{
			// set up a scene such that there is no diffuse term
			Scene test = new Scene();
			test.addPointLight(new PointLight(new Vector3(2, 0, 4), new ColorRGB(1, 1, 1), 4 * Math.PI));
			test.setAmbientLight(new ColorRGB(0, 0, 0));
			Sphere o = new Sphere(new Vector3(0, 0, 2), 1, new ColorRGB(1, 1, 1));
			Vector3 N = new Vector3(0, 0, -1);
			Vector3 P = new Vector3(0, 0, 1);
			...
			check(colour.r == 0 && colour.g == 0 && colour.b == 0);
		}
		finish();
	...
	private void runTestsPart2() {
		...
		if (!wrapper.failed) {
			...
			start("illumination: shadow ray with no occluder");
			{
				Scene test = new Scene();
				test.setAmbientLight(new ColorRGB(0));
				test.addPointLight(new PointLight(new Vector3(1, 0, 0), new ColorRGB(1), 1000));
				test.addObject(new Sphere(new Vector3(3, 0, -2), 1, new ColorRGB(1, 1, 1)));
				...
				Vector3 P = new Vector3(0, 0, 1);
				Vector3 O = new Vector3(0, 0, 0);
				ColorRGB result = wrapper.illuminate(test, o, P, N, O);
				check(result.r>0 & result.g>0 & result.b>0 );
			}
			finish();
	...
	public void test(boolean testAll) {
		...
		if (testAll) {
			...
			System.err.println("Tested only part 1 code. Pass -a to run all the tests.");
		}
		if (failed) {
			...
			System.err.println("PASS: unit tests passed (both parts)");
		}
	}
